...
class DrawConfigCreateForms(ModelSerializer):
    ...
    def create(self, validated_data):
        validated_data["sampler_name"] = "LCM"
        ...
        draw = DrawHistory.objects.create(config=instance, user=self.context["request"].user, image=img_file,
                                          status=status)
...
class UserLikeForm(ModelSerializer):
    ...
    def validate(self, attrs):
        now = datetime.now()
        if UserLike.objects.filter(user=self.context["request"].user, draw_history=attrs["draw_history"],
                                   create_time__day=now.today()).exists():
...
class UserCommentForm(ModelSerializer):
    ...
    def validate(self, attrs):
        if UserComment.objects.filter(user=self.context["request"].user, draw_history=attrs["draw_history"]).count() > 5:
